...
import type {HitOptions, UiSurface} from "@layout/core/surface"
...
describe("input visible geometry", () => {
  ...
  test("maps idle and focused text states without a generic cyan focus owner", () => {
    ...
    expect(idle.roundedRects[0]?.[4]).toMatchObject({
      fill: rgba8ToColor(idleColors.inner),
      border: rgba8ToColor(idleColors.outline),
    })
    ...
    expect(active.roundedRects[0]?.[4]).toMatchObject({
      fill: rgba8ToColor(activeColors.inner),
      border: rgba8ToColor(activeColors.outline),
    })
    expect(active.texts[0]?.[3].material.color).toEqual(rgba8ToColor(activeColors.text))
  ...
  test("keeps grouped cells borderless while active material remains visible", () => {
    ...
    expect(active.roundedRects[0]?.[4]).toMatchObject({
      ...
      border: null,
      borderWidth: 0,
      fill: rgba8ToColor(resolveWidgetColors("text", {selected: true, textInput: true}).inner),
    })
  })
  ...
  test("uses exact selection and caret roles", () => {
    ...
    expect(surface.rects[0]?.[4]).toEqual(rgba8ToColor(uiTheme.widgets.text.item))
    expect(surface.rects[1]?.[4]).toEqual(rgba8ToColor(uiTheme.material.widgetTextCursor))
  })
  ...
  test("draws left, center and right numeric hover zones as secondary results", () => {
    ...
    for (const [zone, pointerX] of [["left", 4], ["center", 50], ["right", 96]] as const) {
      ...
      expect(surface.roundedRects.filter((call) => call[4].z !== Z.ELEMENT - 0.01).slice(1).map((call) => call[4].fill)).toEqual(
        (["left", "center", "right"] as const).map((target) => rgba8ToColor(
      ...
      expect(surface.images[0]![5]!.tint).toEqual(rgba8ToColor(
        resolveNumericZoneColors("number", state, "left")!.colors.item,
      ))
      expect(surface.images[1]![5]!.tint).toEqual(rgba8ToColor(
    ...
    expect(editing.roundedRects[0]?.[4].fill).toEqual(rgba8ToColor(
