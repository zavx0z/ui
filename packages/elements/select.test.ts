...
import {UiSurface as BaseUiSurface, Z} from "@layout/core/surface"
...
import type {DismissableLayerOptions} from "@layout/core/surface"
...
describe("select element", () => {
  test("owns dense value chrome, left label, chevron and caller hit rect", () => {
    ...
    expect(chrome.fill).toEqual(rgba8ToColor(colors.inner))
    expect(chrome.border).toEqual(rgba8ToColor(colors.outline))
    ...
    expect(surface.images[0]![5]!.tint).toEqual(rgba8ToColor(colors.item))
  ...
  test("renders placeholder, active and disabled states without hidden mutation", () => {
    ...
    expect(active.texts[0]?.[3].material.color).toEqual(rgba8ToColor(
      resolveWidgetColors("menu", {inactive: true}).text,
    ))
    const activeColors = resolveWidgetColors("menu", {pressed: true})
    expect(active.roundedRects[0]?.[4].fill).toEqual(rgba8ToColor(activeColors.inner))
    expect(active.roundedRects[0]?.[4].border).toEqual(rgba8ToColor(activeColors.outline))
    ...
    expect(disabled.roundedRects[0]?.[4]).toMatchObject({
      fill: rgba8ToColor(disabledColors.inner),
      border: rgba8ToColor(disabledColors.outline),
    })
    expect(disabled.texts[0]?.[3].material.color).toEqual(rgba8ToColor(disabledColors.text))
  })
  ...
  test("owns hover and pressed visual states", () => {
    ...
    expect(hovered.roundedRects[0]?.[4].fill).toEqual(rgba8ToColor(hoverColors.inner))
    expect(hovered.roundedRects[0]?.[4].border).toEqual(rgba8ToColor(hoverColors.outline))
    ...
    expect(pressed.roundedRects[0]?.[4].fill).toEqual(rgba8ToColor(pressedColors.inner))
    expect(pressed.roundedRects[0]?.[4].border).toEqual(rgba8ToColor(pressedColors.outline))
  })
  ...
  test("opens keyed overlay rows, selects one controlled value and closes after choice", () => {
    ...
    expect(visibleRoundedRects(surface).map((call) => call[4].fill)).toEqual([
      rgba8ToColor(resolveWidgetColors("menu", {pressed: true}).inner),
      rgba8ToColor(resolveWidgetColors("menuBack").inner),
      rgba8ToColor(resolveWidgetColors("menuItem").inner),
      rgba8ToColor(resolveWidgetColors("menuItem", {selectedDraw: true}).inner),
      rgba8ToColor(resolveWidgetColors("menuItem").inner),
      rgba8ToColor(resolveWidgetColors("menuItem", {disabled: true}).inner),
    ])
  ...
  test("renders flat hover and disabled rows inside the one subtle menu border", () => {
    ...
    expect(rows.map(({fill}) => fill)).toEqual([
      rgba8ToColor(resolveWidgetColors("menuItem", {hovered: true}).inner),
      rgba8ToColor(resolveWidgetColors("menuItem", {selectedDraw: true}).inner),
      rgba8ToColor(resolveWidgetColors("menuItem").inner),
      rgba8ToColor(resolveWidgetColors("menuItem", {disabled: true}).inner),
    ])
