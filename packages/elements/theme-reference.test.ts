...
import {uiTheme as publicTheme} from "./theme.ts"
...
describe("source-backed UI theme", () => {
  test("preserves class-specific RGBA bytes and source alpha", () => {
    ...
    for (const [kind, values] of Object.entries(expected)) {
      const set = uiTheme.widgets[kind as keyof typeof uiTheme.widgets]
      expect([
        hex(set.outline),
        hex(set.inner),
  ...
  test("preserves material, status, axes, and Space Node namespaces", () => {
    expect({
      widgetEmboss: hex(uiTheme.material.widgetEmboss),
      menuShadowFactor: uiTheme.material.menuShadowFactor,
      menuShadowWidth: uiTheme.material.menuShadowWidth,
      editorBorder: hex(uiTheme.material.editorBorder),
      editorOutline: hex(uiTheme.material.editorOutline),
      editorOutlineActive: hex(uiTheme.material.editorOutlineActive),
      checkerPrimary: hex(uiTheme.material.checkerPrimary),
      checkerSecondary: hex(uiTheme.material.checkerSecondary),
      checkerSize: uiTheme.material.checkerSize,
      panelRoundness: uiTheme.material.panelRoundness,
      widgetTextCursor: hex(uiTheme.material.widgetTextCursor),
    }).toEqual({
    ...
    expect(Object.fromEntries(Object.entries(uiTheme.state).map(([key, value]) => [key, hex(value)]))).toEqual({
      error: "#771111ff",
      warning: "#ac8737ff",
      info: "#28487dff",
      success: "#188625ff",
    })
    expect(Object.fromEntries(Object.entries(uiTheme.axes).map(([key, value]) => [key, hex(value)]))).toEqual({
      x: "#ff3352ff",
      y: "#8bdc00ff",
      z: "#2890ffff",
    })
    expect({
      back: hex(uiTheme.spaceNode.back),
      header: hex(uiTheme.spaceNode.header),
      navigationBar: hex(uiTheme.spaceNode.navigationBar),
      executionButtons: hex(uiTheme.spaceNode.executionButtons),
      panelHeader: hex(uiTheme.spaceNode.panel.header),
      panelBack: hex(uiTheme.spaceNode.panel.back),
      panelSubBack: hex(uiTheme.spaceNode.panel.subBack),
      tabActive: hex(uiTheme.spaceNode.tab.active),
      tabInactive: hex(uiTheme.spaceNode.tab.inactive),
      tabBack: hex(uiTheme.spaceNode.tab.back),
      tabOutline: hex(uiTheme.spaceNode.tab.outline),
      list: hex(uiTheme.spaceNode.list),
    }).toEqual({
      back: "#1d1d1d00",
      header: "#1d1d1db3",
  ...
  test("deep-freezes raw and resolved tuples while converting Engine colors separately", () => {
    ...
    expectDeepFrozen(uiTheme)
    expectDeepFrozen(resolved)
    expectDeepFrozen(numeric)
    expect(publicTheme).toBe(uiTheme)

    const color = rgba8ToColor([0x47, 0x72, 0xb3, 0xb3])
  ...
  test("resolves an explicit opaque root composite while preserving raw source alpha", () => {
    const raw = uiTheme.spaceNode.back
    const resolved = resolveOpaqueRgba8(raw, uiTheme.spaceNode.navigationBar)
