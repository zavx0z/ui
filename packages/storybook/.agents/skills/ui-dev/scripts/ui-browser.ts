...
type SupportedSelector = Readonly<{
  ...
  pages?: readonly StorybookPageDescriptor[]
...
const registry = await Bun.file(join(import.meta.dir, "storybooks.json")).json() as Registry
const config = resolveTargetConfig(registry, checkout, selectorOrUrl, options)
if (config.selector !== null && !new Set(["targets", "target", "close"]).has(action)) {
...
if (config.selector !== null && config.canvas.capability === "none" && new Set(["canvas", "viewports", "touch", "profile", "interact"]).has(action)) {
  fail(`${action} is unsupported for ${config.selector ?? config.targetUrl}: this storybook has no canvas`)
}
...
function resolveTargetConfig(registry: Registry, checkout: string, input: string, options: Options): TargetConfig {
  if (input.startsWith("http://") || input.startsWith("https://")) {
    const targetUrl = options.route === undefined
      ? new URL(input).href
      : storybookTargetUrl(new URL(input).origin, options.route)
    return {
      checkout,
      selector: null,
      package: null,
  ...
  if (!descriptor) fail(`unknown storybook selector: ${input}`)
  ...
  const pageCanvas = canvasForRoute(descriptor.pages, route, descriptor.canvas)
  return {
    checkout,
    selector: input,
    ...
    canvas: {...pageCanvas, selector: options.canvasSelector ?? pageCanvas.selector},
    testOverride,
  }
}

function canvasForRoute(
  pages: readonly StorybookPageDescriptor[] | undefined,
  route: string,
  fallback: CanvasDescriptor,
): CanvasDescriptor {
  if (pages === undefined) return fallback
  const path = new URL(storybookTargetUrl("http://storybook.invalid", route)).pathname
  const ordered = [...pages].sort((left, right) => right.mountPath.length - left.mountPath.length)
  const page = ordered.find(({mountPath}) => mountPath === "/"
    ? path === "/"
    : path === mountPath || path.startsWith(`${mountPath}/`))
  return page?.canvas ?? fallback
}
