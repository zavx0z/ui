...
# Internal selector-aware dispatcher shared with $nodes-dev.
...
status_json() {
  ...
  if [[ $status == stopped && $last_exit_reason == owner-session-lost ]]; then
    recovery_hint="Run ui-dev.sh ensure in a long-lived PTY before the next lifecycle or browser operation."
  fi
