...
> **[Built for MetaFor](https://github.com/zavx0z/metafor)** as reusable retained WebGPU elements.

`@ui/elements` владеет HTML-like visual primitives, controlled edit behavior,
widget appearance, theme и icons. `@layout/core` отдельно владеет `UiRuntime`,
`UiSurface`, targets, FlexBox, popover chain, text-input dispatch, virtual input
и generic geometry; Elements импортирует exact subpaths и не реэкспортирует их.

Система вёрстки `@layout/core/flex` называется FlexBox: parent описывает flow и
ограничения, общий FlexBox единолично вычисляет local child slots, а child рисует в
...
1. Blender 5.2 LTS является обязательным reference для состава, формы,
...
11. Low-level `@layout/core/surface` drawing capability может иметь не-HTML имя,
    ...
    Elements.
12. Source-backed `UiTheme` хранит raw RGBA и roundness отдельно для widget classes
...
13. Pure `resolveWidgetColors(kind, state)` повторяет class-specific reference
...
15. Resolver precedence детерминирован и повторяет source order. Generic class
    ...
    использует отдельный editor outline. Это mapping не считается reference
...
обязательной consumer boundary Elements, а не вторым owner или реэкспортом.

1. Layout FlexBox заново планирует local child slots только при dirty-изменении
...
1. `@layout/core/surface` предоставляет один low-level rounded shadow primitive поверх
