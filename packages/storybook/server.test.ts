...
describe("@ui/storybook server", () => {
  test("serves a browser entry named client.ts through the public /entry.js route", async () => {
    const root = await mkdtemp(join(tmpdir(), "ui-storybook-server-"))
  ...
  test("mounts independent deep pages with namespaced atomic assets and one build each", async () => {
    const root = await mkdtemp(join(tmpdir(), "ui-storybook-hub-"))
    temporaryRoots.push(root)
    ...
    const alpha = createStorybookPage({
      id: "alpha",
      mountPath: "/alpha",
    ...
    const beta = createStorybookPage({
      id: "beta",
      mountPath: "/beta",
    ...
    try {
      ...
      expect(alphaHtml).toContain('src="/@storybook-assets/alpha/entry.js"')
      expect(betaHtml).toContain("<title>Beta package</title>")
      expect(betaHtml).toContain('<main id="beta-body">Beta body</main>')
      expect(betaHtml).toContain('src="/@storybook-assets/beta/entry.js"')

      const alphaEntryUrl = new URL("/@storybook-assets/alpha/entry.js", server.url)
      const betaEntryUrl = new URL("/@storybook-assets/beta/entry.js", server.url)
      ...
      expect(alphaLazyUrl.pathname).toStartWith("/@storybook-assets/alpha/")
      expect(betaLazyUrl.pathname).toStartWith("/@storybook-assets/beta/")
      ...
      expect(await fetch(new URL("/@storybook-assets/alpha/missing.js", server.url)).then((response) => response.status)).toBe(404)
      expect(await fetch(new URL("/@storybook-assets/missing/entry.js", server.url)).then((response) => response.status)).toBe(404)
  ...
  test("renders Pages shells below /ui with public MetaFor attribution", async () => {
    const root = await mkdtemp(join(tmpdir(), "ui-storybook-pages-shell-"))
    temporaryRoots.push(root)
    const entrypoint = join(root, "client.ts")
    const stylePath = join(root, "style.css")
    await Promise.all([
      Bun.write(entrypoint, 'document.documentElement.dataset.staticPage = "ready"'),
      Bun.write(stylePath, "html { background: black }"),
    ])
  ...
  test("serves only registered route-tree overviews and leaves with canonical redirects", async () => {
    const root = await mkdtemp(join(tmpdir(), "ui-storybook-route-tree-"))
    temporaryRoots.push(root)
    const entrypoint = join(root, "client.ts")
    const stylePath = join(root, "style.css")
    ...
    const catalog = createStorybookPage({
      id: "catalog",
      mountPath: "/",
      packageName: "Catalog",
      entrypoint,
    ...
    const ui = createStorybookPage({
      ...
      homePath: "/",
      routeTree: defineStorybookRouteTree({
        leaves: ["button/basic/contained", "button/basic/text"] as const,
      }),
    })
    expect(() => createStorybookPage({
      id: "invalid",
      mountPath: "/invalid",
      packageName: "Invalid",
      entrypoint,
    ...
    const server = startStorybookHubServer({pages: [catalog, ui], hostname: "127.0.0.1", port: 0})
    ...
    try {
      ...
      for (const pathname of [
        ...
        if (pathname === "/") expect(html).not.toContain("data-storybook-home")
        else {
