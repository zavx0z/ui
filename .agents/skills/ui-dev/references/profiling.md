...
Read this reference only when a UI storybook needs performance or GPU evidence.
...
Inspector remains an external diagnostic mode. Keep the clean target open and
create one separate instrumented target for the exact standalone Storybook
origin. Do not add Inspector scripts, bridge, extension, package, loader,
overlay, or capture API to UI source, HTML, runtime, build, or browser bundle.
