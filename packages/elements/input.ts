...
function handleActiveInputKey(surface: UiSurface, event: KeyboardEvent): boolean {
...
function surfaceHasActiveInput(surface: UiSurface): boolean {
  const runtime = inputRuntime.get(surface)
...
export function input(surface: UiSurface, x: number, y: number, width: number, height: number, props: InputProps): void {
  ...
  if (active) drawInputSelection(
    ...
    chrome.height,
    fontPx,
    rgba8ToColor(uiTheme.widgets[widgetClass].item),
  )
  ...
  if (text.length > 0) {
    surface.drawText(text, display.originX, chrome.y + (chrome.height - fontPx) / 2, {
      fontPx,
      material: textMaterial(surface, style.color ?? rgba8ToColor(
  ...
  if (active && props.cursorVisible !== false && runtime.caretVisible) {
    ...
    surface.drawRect(
      ...
      2,
      Math.max(1, fontPx + 2),
      rgba8ToColor(uiTheme.material.widgetTextCursor),
      Z.TEXT + 0.02,
    )
  }
  surface.popClip()
...
function inputRuntimeFor(surface: UiSurface): InputRuntimeState {
  ...
  if (runtime === undefined) {
    ...
    registerTextInputController(surface, {
      handleKey: (event) => handleActiveInputKey(surface, event),
      insertText: (text) => insertActiveInputText(surface, text),
      hasActiveInput: () => surfaceHasActiveInput(surface),
      blur: (nextHitKey) => blurActiveInput(surface, nextHitKey),
    })
  }
  return runtime
}
...
function drawNumericInputZone(
  ...
  for (const target of ["left", "center", "right"] as const) {
    ...
    surface.drawRoundedRect(x, chrome.y, width, chrome.height, {
      radius,
      fill: rgba8ToColor(zone.colors.inner),
      border: null,
      borderWidth: 0,
      z: Z.ELEMENT + 0.01,
    })
    if (target !== "center") {
      drawIconCentered(
        ...
        {tint: rgba8ToColor(zone.colors.item), z: Z.TEXT},
      )
    }
  }
}
