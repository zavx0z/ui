...
describe("central ui-dev registry", () => {
  test("publishes the one standalone UI selector", async () => {
    ...
    expect(Object.keys(registry.selectors)).toEqual(["ui"])
    expect(registry.selectors.ui).toMatchObject({
      supported: true,
      package: "@ui/storybook",
      cwd: "packages/storybook",
      port: 4017,
      command: ["bun", "app/server.ts"],
      httpMarker: "<title>UI storybook</title>",
  ...
  test("preserves canonical overview slashes and exact leaves", () => {
    expect(storybookTargetUrl("http://127.0.0.1:4017", "/"))
      .toBe("http://127.0.0.1:4017/")
    expect(storybookTargetUrl("http://127.0.0.1:4017", "/elements/"))
      .toBe("http://127.0.0.1:4017/elements/")
    expect(storybookTargetUrl("http://127.0.0.1:4017", "/components/button/basic/contained"))
      .toBe("http://127.0.0.1:4017/components/button/basic/contained")
    expect(() => storybookTargetUrl("http://127.0.0.1:4017", "components/button"))
      .toThrow("absolute pathname")
    expect(() => storybookTargetUrl("http://127.0.0.1:4017", "/components//button"))
      .toThrow("normalized pathname")
    expect(() => storybookTargetUrl("http://127.0.0.1:4017", "/components/button?mode=hash"))
      .toThrow("normalized pathname")
  })
  ...
  test("keeps browser control background-only, route-aware and data-only", async () => {
    ...
    expect(source).toContain("this storybook has no canvas")
...
describe("central ui-dev lifecycle", () => {
  test("wrapper selects the UI catalog and dispatcher owns one exact process", async () => {
    ...
    try {
      ...
      expect(status.cwd).toBe(join(checkout, "packages/storybook"))
      ...
      expect(logs.stdout).toContain("[UI storybook catalog]")
  ...
  test("restart replaces only the exact UI PID", async () => {
    ...
    try {
      ...
      expect(second).toMatchObject({selector: "ui", package: "@ui/storybook"})
